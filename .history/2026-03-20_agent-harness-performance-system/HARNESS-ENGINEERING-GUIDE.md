...
nyosegawa（逆瀬川）は2026年3月の記事で、これらの知見を体系化し7つの原則として整理しています（nyosegawa.com, 2026-03-09）。

1. **リポジトリ衛生** — 腐敗を前提に設計する
2. **決定論的ツールとアーキテクチャガードレール** — 品質を強制する
3. **AGENTS.md はポインタとして設計する** — ルールを直接書くのではなくスキルや ADR、リンター設定への参照だけを置き、50行以下に収める
4. **計画と実行の分離**
5. **E2E テスト戦略**
6. **セッション間の状態管理**
7. **プラットフォーム固有のハーネス戦略** — Codex のクラウドサンドボックス型と Claude Code のローカル Hook 型では設計が異なるため、各プラットフォームの特性を活かす

このうち最も重要な構造的洞察は「フィードバック速度の階層」です。PostToolUse Hook によるミリ秒単位のフォーマット自動適用が最速で、プリコミットフック（秒）、CI パイプライン（分）、人間によるコードレビュー（時間〜日）と続きます。ハーネスエンジニアリングの目標は、できる限り多くのチェックをより速い層に移動させることです。
...
| カテゴリ | ID | 評価対象 | 配点 |
|---------|-----|---------|------|
| **Tool Coverage** | `tool-hooks-config` | `hooks/hooks.json` が存在し、Hook イベント定義（PostToolUse, PreToolUse, Stop 等）がある | 2 |
| | `tool-hooks-impl-count` | `scripts/hooks/` に 8 件以上の Hook 実装スクリプトがある（post-edit-format, quality-gate, session-start 等） | 2 |
| | `tool-agent-count` | `agents/` に 10 件以上のエージェント定義がある（code-reviewer, security-reviewer, tdd-guide 等） | 2 |
| | `tool-skill-count` | `skills/` に 20 件以上の SKILL.md 定義がある（tdd-workflow, e2e-testing, eval-harness 等） | 2 |
| | `tool-command-parity` | `commands/` と `.opencode/commands/` の harness-audit コマンドが同一内容で同期されている | 2 |
| | | **小計** | **10** |
| **Context Efficiency** | `context-strategic-compact` | `skills/strategic-compact/SKILL.md` による戦略的コンパクション指針がある（論理的区切りでの実行、自動コンパクション抑制） | 3 |
| | `context-suggest-compact-hook` | `scripts/hooks/suggest-compact.js` が存在し、ツール呼び出し回数の閾値（デフォルト50回）でコンパクト提案を行う | 3 |
| | `context-model-route` | `commands/model-route.md` によるタスク複雑度に応じたモデル選択ガイド（haiku/sonnet/opus の使い分け） | 2 |
| | `context-token-doc` | `docs/token-optimization.md` にモデル選択戦略、thinking tokens 設定、autocompact 閾値、サブエージェントモデル設定を記載 | 2 |
| | | **小計** | **10** |
| **Quality Gates** | `quality-test-runner` | `tests/run-all.js` による一括テスト実行エントリポイントがある | 3 |
| | `quality-ci-validations` | `package.json` の test スクリプトが `validate-commands.js`（コマンド整合性検証）と `tests/run-all.js` の双方を実行する | 3 |
| | `quality-hook-tests` | `tests/hooks/hooks.test.js` が存在し、Hook の振る舞い（matcher 照合、プロファイル制御、出力形式）を検証する | 2 |
| | `quality-doctor-script` | `scripts/doctor.js` によるインストール状態の健全性チェック（ファイル欠損、設定ドリフト検出）がある | 2 |
| | | **小計** | **10** |
| **Memory Persistence** | `memory-hooks-dir` | `hooks/memory-persistence/` ディレクトリが存在し、メモリライフサイクル Hook 定義がある | 4 |
| | `memory-session-hooks` | `scripts/hooks/session-start.js`（前セッション文脈ロード）と `scripts/hooks/session-end.js`（状態永続化・パターン評価）の両方が存在する | 4 |
| | `memory-learning-skill` | `skills/continuous-learning-v2/SKILL.md` による継続学習フロー（instinct 抽出 → skill 昇格 → 知識進化）がある | 2 |
| | | **小計** | **10** |
| **Eval Coverage** | `eval-skill` | `skills/eval-harness/SKILL.md` による形式的評価フレームワーク（EDD 原則、capability/regression eval、code/LLM/hybrid grader、pass@k） | 4 |
| | `eval-commands` | `commands/eval.md`（binary/scalar/rubric grader 実行）、`commands/verify.md`（型チェック＋lint＋テスト＋ビルドの検証ループ）、`commands/checkpoint.md`（状態スナップショット保存）の 3 コマンドすべてが存在する | 4 |
| | `eval-tests-presence` | `tests/` に 10 件以上の `.test.js` ファイルが存在する（Hook テスト、ライブラリテスト、スクリプトテスト、CI テスト等） | 2 |
| | | **小計** | **10** |
| **Security Guardrails** | `security-review-skill` | `skills/security-review/SKILL.md` によるセキュリティレビュースキル（OWASP Top 10、クラウドインフラセキュリティ、SSRF/インジェクション検出） | 3 |
| | `security-agent` | `agents/security-reviewer.md` による専任セキュリティレビューエージェント（脆弱性検出・修復提案の委譲先） | 3 |
| | `security-prompt-hook` | `hooks/hooks.json` に `beforeSubmitPrompt`（プロンプト内シークレット検出: sk-, ghp_, AKIA パターン）または `PreToolUse` ガードのイベント参照がある | 2 |
| | `security-scan-command` | `commands/security-scan.md` によるセキュリティスキャン＆修復ワークフローコマンドがある | 2 |
| | | **小計** | **10** |
| **Cost Efficiency** | `cost-skill` | `skills/cost-aware-llm-pipeline/SKILL.md` によるコスト最適化パターン（予算ルーティング、トークン節約テクニック） | 4 |
| | `cost-doc` | `docs/token-optimization.md` にモデル選択コスト比較、MAX_THINKING_TOKENS 設定（~70%削減）、AUTOCOMPACT_PCT 設定を記載 | 3 |
| | `cost-model-route-command` | `commands/model-route.md` による複雑度別モデルルーティング（haiku=最安/決定論的、sonnet=デフォルト、opus=設計判断）と予算フラグ（low/med/high） | 3 |
| | | **小計** | **10** |
| | | **合計** | **70** |
...
ここでのポイントは2つあります。まず自動修正可能なものは先に修正し（`biome format --write`, `oxlint --fix`）、エージェントに返すのは残った違反のみにすることです。次に、フィードバックの返し方として、この例では `hookSpecificOutput.additionalContext` を含む JSON 形式を使用しています。Claude Code の Hook システムでは `exit 0` でプレーンテキストを stdout に出力するだけでもコンテキストに注入されますが、JSON 形式を使うことで `permissionDecision` 等の構造化された判定と組み合わせられる利点があります。
...
|-------|---------|-------------|---------|
| テストカバレッジ向上 | continuous-pr | 「テストされていない全関数にユニットテストを追加して」 | `--max-runs 10` or カバレッジ80%到達 |
| リント違反の一掃 | sequential | 「ESLint エラーをすべて修正して。修正のたびに lint を実行して」 | スクリプト完了 |
| PRD からの機能実装 | rfc-dag | PRD ファイルを入力に、ユニット分解→並列実装 | 全ユニットの acceptance criteria 通過 |
| フレームワーク移行 | continuous-pr | 「Express から Hono に移行して。1イテレーションにつき1ルートずつ」 | `--max-cost $50` |
| デプロイ監視 | sequential (cron) | 「デプロイの健全性をチェックして。エラーがあれば修正 PR を作成して」 | 5分間隔で継続 |
