...
function parseZhDocsReadmeExpectations(readmeContent) {
  ...
  const tablePatterns = [
    { category: 'agents', regex: /\|\s*智能体\s*\|\s*✅\s*(\d+)\s*个\s*\|/i, source: 'docs/zh-CN/README.md comparison table' },
    { category: 'commands', regex: /\|\s*命令\s*\|\s*✅\s*(\d+)\s*个\s*\|/i, source: 'docs/zh-CN/README.md comparison table' },
    { category: 'skills', regex: /\|\s*技能\s*\|\s*✅\s*(\d+)\s*项\s*\|/i, source: 'docs/zh-CN/README.md comparison table' }
  ];
...
function collectExpectations(documentSpecs) {
  return documentSpecs.flatMap(spec => spec.parseExpectations(readFileOrThrow(spec.filePath)));
}
...
function renderText(result) {
  if (result.updatedFiles.length > 0) {
    console.log(`Updated documentation counts in ${result.updatedFiles.length} file(s):`);
...
function main() {
  ...
  const result = { catalog, checks, updatedFiles };
